document.addEventListener("DOMContentLoaded", function() {
    ...
    function engineGame(options) {
        ...
        function updateScoreBar(score) {
            ...
            if (!scoreBar) return; // Exit if score bar doesn't exist
        ...
        // Material score calculation function
        function calculateMaterialScore() {
            console.log('Calculating material score...');
            let material = { white: 0, black: 0 };
            const pieceValues = { 'p': 1, 'n': 3, 'b': 3, 'r': 5, 'q': 9, 'k': 0 };
            
            const fen = game.fen().split(' ')[0]; // Get just the board position
            for (let char of fen) {
                if (char === '/') continue;
                ...
                if (char === char.toUpperCase()) {
                    // White piece
                    material.white += pieceValues[char.toLowerCase()] || 0;
                } else {
                    // Black piece
                    material.black += pieceValues[char] || 0;
                }
            }
            
            const materialDifference = material.white - material.black;
            console.log(`Material Score - White: ${material.white}, Black: ${material.black}, Difference: ${materialDifference > 0 ? '+' : ''}${materialDifference}`);
        ...
        return {
            ...
            reloadWithFEN: function(fen) {
                ...
                // Calculate and log material score for the new position
                calculateMaterialScore();
                ...
                if (evaluation_el) evaluation_el.textContent = '';
                if (engineAnalysisEl) engineAnalysisEl.textContent = '';
    ...
    function adjustScoreBarHeight() {
        ...
        if (!boardElement || !scoreBarContainer) return; // Exit if elements don't exist
        
        const boardHeight = boardElement.offsetHeight;
        scoreBarContainer.style.height = `${boardHeight}px`;
    }

    // Only run if score bar elements exist
